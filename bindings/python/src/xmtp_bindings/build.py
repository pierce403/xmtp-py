...
def _clone_libxmtp(repo_url: str, dest: Path, ref: str | None, announce: Callable[[str], None]) -> None:
    _ensure_tool('git')
    if ref:
        try:
            _run(['git', 'clone', '--depth', '1', '--branch', ref, repo_url, str(dest)], cwd=dest.parent)
            return
...
def _ensure_libxmtp(announce: Callable[[str], None]) -> None:
    ...
    libxmtp_dir = Path(libxmtp_path).expanduser().resolve() if libxmtp_path else deps_dir / 'libxmtp'
    ...
    if not libxmtp_dir.exists():
        ...
        _run(['git', '-C', str(libxmtp_dir), 'fetch', '--depth', '1', 'origin', ref], cwd=libxmtp_dir)
